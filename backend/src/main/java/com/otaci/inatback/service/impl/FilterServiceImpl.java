...
public class FilterServiceImpl implements IFilterService {
    ...
    private List<PriceIntervalDto> buildPriceIntervalsInternal(List<Long> categoryIds) {
    ...
    private List<PriceIntervalDto> buildNicePriceIntervals(
            double min,
            double max,
            int bucketCount
    ) {
    ...
    public List<PriceIntervalDto> getPriceIntervalsByCategory(Long categoryId) {
        ...
        return buildPriceIntervalsInternal(categoryIds);
    }
    ...
    /* -------------------------------------------------
       PUBLIC API – SEARCH MODE
     ------------------------------------------------- */

    public List<PriceIntervalDto> buildPriceIntervalsForCategories(
            List<Long> categoryIds
    ) {
        return buildPriceIntervalsInternal(categoryIds);
    }
    ...
    public List<ProductResponse> filterProducts(
        ...
        List<Product> products =
                (brands != null && !brands.isEmpty())
                        ? productRepository
                        .findByCategoryIdInAndBrandInAndDeletedFalse(
                                categoryIds,
                                brands
                        )
                        : productRepository
                        .findByCategoryIdInAndDeletedFalse(categoryIds);

        List<ProductResponse> result = new ArrayList<>();

        for (Product product : products) {

            List<ProductVariant> matched = product.getVariants().stream()
                    .filter(v -> {
